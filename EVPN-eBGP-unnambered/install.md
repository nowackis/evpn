...
## Lab topology
![EVPN L3 Topology](EVPN_LAB_L2-ebgp-multi_vrf.png)
...
In this phase, we build the **underlay IP fabric**. This involves deploying FRR containers and establishing the physical-like "cabling" between nodes.
...
* Run eBGP underlay only.
* Provide IP transport between Leaf nodes.
* Do not participate in EVPN or VXLAN overlay functions.

**Leafs (LEAF_1, LEAF_2)**
* Run eBGP toward spines to establish the fabric.
* Advertise loopback addresses (3.3.3.3 and 4.4.4.4) for VTEP identification.
* Will later act as VTEPs (VXLAN Tunnel End Points) for the overlay.
...
This part build EVPN using configuration from gitlab: [EVPN-eBGP-unnambered
](https://gitlab.datacrunch.io/networking/misc/-/tree/master/EVPN/EVPN-eBGP-unnambered?ref_type=heads)
